...
function Optometrist() {
    ...
    const searchOptometrist = async (e, x) => {
        if (e) {
            setLoading(true);
            let q = x || 1;
            const data = { payload: e?.trim(), page: q, limit: 10 }
            try {
        ...
        if (searchArray.length > 0) {
            return (
                <tr key={i}>
                    ...
                    {/* <td>{e.status?.toLowerCase() === "approved" || e.status?.toLowerCase() === null ?
                        <span className="ri-checkbox-circle-line align-middle text-success"><span className='mx-1'>Up To Date</span></span> :
                        <span className="ri-close-circle-line align-middle text-danger"><span className='text-dark mx-1'>Not Up To Date</span></span>
                    }
                    </td> */}
                    ...
                    {/* <td>
                        <div className={e.userId === null ? 'hstack flex-wrap d-none' : 'hstack flex-wrap'}>
                            <Link to={`/search-dashboard/view/${`optometrist`}/${e.id}`} className="link-success btn-icon btn-sm" id="Tooltip3"><i className="ri-compass-3-line fs-16"></i></Link>
                        </div>
    ...
    const handlePagination = page => {
        searchOptometrist(comment, page.selected + 1);
                    ...
                    <div className="col-xl-12">
                        <div className="">
                            <Row className=' p-2 mb-3'>
                                <Col>
                                    <h4 className="card-title mb-0 p-2">Registered Optometrists </h4>
                                </Col>

                                <Col lg={7} className="mb-2">
                                    <div className="form-group m-0" style={{ width: '90%' }}>
                                        <div className="input-group">
                                            <input type="text" className="form-control" onChange={e => {
                                                setComment(e.target.value);
                                                searchOptometrist(e.target.value, 1);
                                            }}
                                ...
                                <Col sm={2} className="mb-2">
                                    <div className='px-2'>
                                        <button onClick={() => searchOptometrist(comment, 1)} className="btn text-white" style={{ background: '#05B604' }} type="button"><i
